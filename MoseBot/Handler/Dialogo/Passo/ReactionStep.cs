...
{
    internal class ReactionStep : DialogueStepBase
    {
        ...
        public override async Task<bool> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            {
                Title = "Porfavor Reaja",
                Description = $"{user.Mention}, {_content}",
            };

            embedBuilder.AddField("Para parar o dialogo", "Reaja com :x: emoji");

            var interatividade = client.GetInteractivity();
            ...
            while (true)
            {
                ...
                var reactionResult = await interatividade.WaitForReactionAsync(
                    x => _options.ContainsKey(x.Emoji) ||
                    x.Emoji == cancelEmoji,
                    embed,
