...
{
    internal class TextStep : DialogueStepBase
    {
        ...
        public override async Task<bool> ProcessStep(DiscordClient client, DiscordChannel channel, DiscordUser user)
        {
            ...
            {
                embedBuilder.AddField("Quantidade Minima de Caracteres: ", $"{_minLength.Value}");
            }
            if (_maxLength.HasValue)
            {
                embedBuilder.AddField("Quantidade Maxima de Caracteres: ", $"{_maxLength.Value}");
            }
            ...
            while (true)
            {
                ...
                if (_minLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Sua mensagem de {_minLength.Value - messageResult.Result.Content.Length} caracteres é muito pequena").ConfigureAwait(false);
                        continue;
                    }
                }
                if (_maxLength.HasValue)
                {
                    ...
                    {
                        await TryAgain(channel, $"Sua mensagem de {messageResult.Result.Content.Length - _maxLength.Value} caracteres é muito grande").ConfigureAwait(false);
                        continue;
                    }
                }
